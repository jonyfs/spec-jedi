<!-- i18n-sync: source=README.md@2a01f98 lang=ar -->
...
[![Skills](https://img.shields.io/badge/specjedi_skills-30_shipped-success)](#كيف-يطبّق-spec-jedi-منهج-sdd)
...
الترتيب. قبل وجود أي سطر كود، يُدوَّن ما يُبنى ولماذا، في أربع وثائق
منظَّمة قابلة للمراجعة: **دستور** 📜 يكتبه `specjedi-constitution`
(القواعد غير القابلة للتفاوض الخاصة بالمشروع)، و**مواصفة** 🎯 يكتبها
`specjedi-specify` (ماذا، ولمن)، و**خطة** 🛠️ يكتبها `specjedi-plan` (كيف
تقنياً)، و**قائمة مهام** ✅ يكتبها `specjedi-tasks` (الخطوات المرتبة).
يُولَّد الكود *بناءً* على هذه الوثائق الأربع، وليس العكس. شرح كامل، دون
أي علامة تجارية خاصة بـ Spec Jedi:
...
‏Spec Jedi هي **منافس** لـ [spec-kit](https://github.com/github/spec-kit)
([Principle XV](../../../.specify/memory/constitution.md))، ومبنية كحزمة
مهارات `specjedi-*` يُثبّتها المشروع جنباً إلى جنب مع أوامر `speckit-*`
الخاصة بـ spec-kit — أو بدلاً منها — مع دعم كامل لعشرين وكيل برمجة
مستهدَفاً (راجع [التثبيت](#التثبيت) أدناه). سلسلة `specjedi-*` الكاملة
لتطوير SDD — من الدستور إلى التقارب — مُسلَّمة بالكامل منذ فترة: جميع
المراحل التسع، كل واحدة مبنية على بحث تنافسي حقيقي قبل كتابة أول سطر منها
...
تتوفر اليوم ثلاثون skill إجمالاً، عبر أربعة تخصصات — الكتالوج الكامل،
...
### `specjedi-*` مقابل `speckit-*`، بالأرقام

مقارنة قائمة على الأدلة، أمراً بأمر —
[`specs/044-speckit-parity-audit/PARITY-LEDGER.md`](../../../specs/044-speckit-parity-audit/PARITY-LEDGER.md)
— فحصت كل واحد من أوامر `speckit-*` الأحد عشر في مسارها مقابل نظيره من
`specjedi-*` بناءً على السلوك الموصوف فعلياً، لا تشابه الأسماء:

- **8 من 11** في تكافؤ كامل — نفس المهمة، نفس المدخلات/المخرجات.
- **1 من 11** (`specjedi-implement` مقابل `speckit-implement`) هو اختلاف
  لصالح `specjedi-implement`: يشترط `specjedi-implement` الالتزام
  (commit) فقط عبر فرع ميزة وطلب سحب، وأبداً مباشرة على الفرع الرئيسي؛
  بينما لا تتضمن تعليمات `speckit-implement` نفسها أي انضباط لفروع git أو
  الالتزامات على الإطلاق.
- **2 من 11** ليس لهما نظير في `specjedi-*` — وكلاهما مُسوًّى بالفعل،
  وليس ثغرة مفتوحة: تحويل المهام إلى مشكلات (issues) في GitHub (لم
  يُستخدَم أبداً في التاريخ الفعلي لهذا المشروع)، ومؤشر "الخطة الحالية"
  الدائم (استُبدل بتصميم `specjedi-status` الذي لا يتتبع مسارات متوازية،
  وبإعادة إظهار حالة بداية الجلسة الخاصة بـ Constitution Principle XXI).
- **21 من 30** مهارة `specjedi-*` مُسلَّمة ليس لها أي نظير في `speckit-*`
  على الإطلاق — `specjedi-catalog-audit`، و`specjedi-chain`،
  و`specjedi-constitution-audit`، و`specjedi-diagram`، و`specjedi-docs`،
  و`specjedi-explain`، و`specjedi-find-skills`، و`specjedi-govcheck`،
  و`specjedi-master`، و`specjedi-migrate`، و`specjedi-new-skill`،
  و`specjedi-onboard`، و`specjedi-parallel`، و`specjedi-quick`،
  و`specjedi-release`، و`specjedi-retro`، و`specjedi-security`،
  و`specjedi-skill-review`، و`specjedi-status`، و`specjedi-tokencheck`،
  و`specjedi-worktree` — قدرة إضافية حقيقية، لا إعادة صياغة للمسار نفسه.
...
> ⚠️ **يعرض هذا القسم مسار التطوير الفعلي والحقيقي لهذا المشروع بنفسه قيد
> العمل** — أوامر `specjedi-*` أدناه هي سطح المنتج الحقيقي نفسه الموصوف
> أعلاه، مُستخدَمة على Spec Jedi ذاته. حتى الميزة رقم 048 (2026-07-18)،
> كان هذا المشروع يُمهِّد نفسه بأوامر `speckit-*` الموزَّعة (vendored)
> الخاصة بـ spec-kit (نفس نمط "تمهيد مترجم بمترجم أقدم" الذي قد يستخدمه
> أي منافس أثناء بناء بديله) قبل أن يكتمل مسار `specjedi-*` الخاص به بما
> يكفي لتولّي المهمة. انتهت مرحلة التمهيد تلك — راجع
> [Principle XV](../../../.specify/memory/constitution.md) للسياسة
> الكاملة التي أنجزت هذا الانتقال.
>
...
تكون أبداً الأخيرة. `/specjedi-constitution` تكتب القواعد مرة واحدة، حتى
...
يُبنى فعلياً، ولمن. `/specjedi-specify` تحوّلها إلى `spec.md` حقيقي؛
`/specjedi-clarify` تنطلق لتطارد الغموض قبل أن يتحول إلى خلل لا يريد أحد
...
ثم يظهر المخطط. `/specjedi-plan` يصبح `plan.md`، و`/specjedi-tasks` يقسّمه
...
تدريجياً، تتوقف عن الفشل. `/specjedi-implement` تنفّذ `tasks.md` مع
...
لا شيء من هذا قصة خيالية. إنه العملية الحرفية والمتكررة وراء طلبات السحب
...
بالنسبة لـ `claude-code`، و`codex-cli`، و`trae`، يُنشئ المثبِّت أيضاً أو
يحدِّث ملف ذاكرة المشروع الخاص بتلك البيئة (`CLAUDE.md`، أو `AGENTS.md`،
أو `.trae/rules/project_rules.md`) بقسم قصير محدَّد بعلامات (markers)
يسمّي المهارات المثبَّتة — بحيث يعرف سياق البيئة نفسها أنها موجودة، وليس
فقط دليل المهارات ذاته. إذا كان ذلك الملف موجوداً بالفعل بمحتواك الخاص،
لا يُمَسّ أي شيء منه؛ يُضاف القسم الجديد إليه، ويُحدَّث ذلك القسم فقط في
عمليات إعادة التثبيت اللاحقة.
...
بالعرض قبل إظهارها، وأربعة إصدارات حقيقية صدرت بالفعل (`v0.1.0`،
و`v0.1.1`، و`v0.2.0`، و`v0.3.0`)، والدستور وثيقة حية ذات إصدار عند
v1.29.0 مع سجل تعديلات موثَّق. النصف الآخر، بصراحة: معظم مسارات تثبيت
بيئات الجسر تستند إلى بحث مكتبي، وليس إلى جلسة عملية فعلية داخل المنتج
الحقيقي لطرف ثالث (راجع أدناه)، وتوثيق هذا المشروع المترجَم محلياً يتأخر
عن مصدره الإنجليزي في أغلب الأحيان — يُبلِّغ `scripts/validate.sh` حالياً
عن كل النسخ العشر المترجَمة من `README.md` و`CONTRIBUTING.md` بأنها غير
متزامنة مع أحدث التغييرات الإنجليزية، وهي ثغرة حقيقية ومتكررة لم يُغلقها
هذا المشروع بعد بشكل بنيوي. الصورة الكاملة، دون تنقيح:
