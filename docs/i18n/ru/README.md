<!-- i18n-sync: source=README.md@1609524 lang=ru -->
...
[![Skills](https://img.shields.io/badge/specjedi_skills-25_shipped-success)](#как-spec-jedi-реализует-sdd)
...
записывается, что строится и почему, в виде структурированного,
проверяемого документа — **constitution** 📜 (неотъемлемые правила),
**specification** 🎯 (что и для кого), **plan** 🛠️ (как именно,
технически) и **task list** ✅ (упорядоченные шаги). Код генерируется *на
основе* этих артефактов, а не наоборот — та же дисциплина, которую
Кодекс джедаев требует от того, кто соблазнён пропустить скучные части
обучения. Полное объяснение, без единой собственной марки Spec Jedi:
...
обёртка вокруг него
([Principle XV](../../../.specify/memory/constitution.md)) — двадцать
кодовых агентов поддерживаются по-настоящему, а не только в теории (см.
...
только через feature-ветку и pull request. Всего сегодня доступно
двадцать пять навыков в четырёх дисциплинах — полный каталог, обе
диаграммы и пошаговое руководство из 23 шагов живут в
...
> [Установка](#установка) ниже** — реальная поверхность продукта — это
> навыки `specjedi-*`, а не эти. См.
> [Principle XV](../../../.specify/memory/constitution.md) для полной
> политики о том, почему они чётко разделены.
>
...
`/speckit-constitution` записывает правила один раз, чтобы никому не
...
`/speckit-specify` превращает её в настоящий `spec.md`;
`/speckit-clarify` отправляется выслеживать неоднозначность прежде, чем
она станет багом, который никто не захочет признавать своим.
...
Затем появляется чертёж. `/speckit-plan` становится `plan.md`,
`/speckit-tasks` разбивает его на упорядоченный, учитывающий
...
потом, постепенно, перестают падать. `/speckit-implement` выполняет
...
Ничто из этого не выдумка — это буквальный, повторяющийся процесс за
недавними pull request'ами самого этого проекта —
...
страница. Двадцать из двадцати целевых окружений имеют реальный путь
установки, проверенный CI, диаграммы проверяются рендерингом прежде,
чем показаны, а constitution — живой, версионируемый документ версии
v1.24.0 с задокументированной историей поправок. Вторая половина,
сказанная прямо: ни один релиз ещё не выпущен (`git tag -l` на момент
написания не возвращает ничего), и большинство путей установки для
мостовых окружений опираются на кабинетное исследование, а не на
практическую сессию внутри реального стороннего продукта. Полная,
нефильтрованная картина:
