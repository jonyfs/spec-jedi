<!-- i18n-sync: source=README.md@bf963a8 lang=zh -->
...
[![Skills](https://img.shields.io/badge/specjedi_skills-24_shipped-success)](#今天你能获得什么)
...
这个"教团"共有二十四名成员——训练的不是战斗，而是规格驱动开发。它遵循四大门类：
...
| `specjedi-quick` ⚡ | 面向小型、已充分理解的改动的轻量路径——只需一份 `quick.md`，无需 `spec.md`+`research.md`+`plan.md`+`tasks.md`，直接进入实现。质量关卡（测试先行、`specjedi-govcheck`、仅通过 PR）从不缩减，缩短的只是规划环节。遇到更大、含糊、涉及多个子系统，或需要新建技能的改动，会拒绝并引导至 `specjedi-specify`——参见[该使用哪条路径](#该使用哪条路径) |
...
3. 在 Claude Code 提示符中输入 `/` 确认技能已加载。你会看到全部 24 个 `specjedi-*` 产品技能，
...
**完全不想克隆本仓库？** `scripts/bootstrap-install.sh`/`.ps1`
（specs/024-bootstrap-installer）会为你拉取一个已发布的 GitHub Release，并运行其中
打包好的安装程序——无需本地检出：
...
⚠️ 本项目自己的第一个发布版本尚未剪切（Principle XI——剪切发布始终是维护者有意为之的
一步，绝不自动发生），因此上面这条一键命令目前会报告"未找到发布版本"（no release
found），并给出 git-clone 的回退命令。它已经交付并在这一真实的当前状态下通过了 CI
测试；一旦有了正式发布版本，它就会开始真正执行安装。

`--harness` 是可选的——如果省略，安装程序会尝试在 `claude-code`/`codex-cli`/`trae` 之间
检测你正在使用的编码智能体（已存在的项目目录、`PATH` 中的可执行文件，或已存在的全局配置
目录），并自动为其安装；只有在检测到多个可能匹配时才会询问。其余 17 个宿主环境（目前还
没有可靠的文件系统/PATH 检测信号）需要显式传入 `--harness`。运行 `./scripts/install.sh
--help`（或 `.\scripts\install.ps1 -Help`）查看完整选项列表，包括 `--auto`。

### 受支持的宿主环境

Spec Jedi 的章程（[Principle III](../../../.specify/memory/constitution.md)）承诺该项目
支持市场上使用率最高的二十种 LLM 编码工具/宿主环境——截至本次发布，全部二十个都是真实、
经过测试、CI 验证过的。其中四个采用原生的 skills 目录扫描（Claude Code、Codex CLI、
Trae、Antigravity——后三者只共享两个物理目标目录，`.agents/skills/` 和
`.trae/skills/`，此外 OpenCode 和 Warp 无需额外代码即可通过这些相同路径得到满足）。
其余十四个没有原生的 skills 目录概念——只有一个项目根目录规则文件、一个小型规则目录，
或者（对 Sourcegraph Cody 而言）一个自定义命令 JSON 文件——因此安装程序会生成一个
**桥接文件**：完整的 `specjedi-*` 包仍然落在标准位置 `.claude/skills/`，再由一个小的
适配文件（对于目录式宿主环境，则是每个技能一个文件）用该宿主环境自己文档化的约定指向
它。每个宿主环境确切机制背后的引用出处，见
...
按照 Principle III"至少二十种"的要求，逐一列出了二十种宿主环境，全部 ✅ 已支持——对本
项目尚未实际构建和测试过的机制不作任何能力方面的断言，遵循 Principle XX 的抗幻觉纪律。

参见 [`references/harness-capability-notes.md`](../../../references/harness-capability-notes.md)
获取每个宿主环境最初的桌面调研能力说明，以及
[`specs/023-full-harness-coverage/research.md`](../../../specs/023-full-harness-coverage/research.md)
获取本表格所依据的安装机制决策与引用出处。

好奇 Spec Jedi 与 spec-kit 及其对标的另外十款 SDD 工具相比如何？参见
[`references/competitive-comparison.md`](../../../references/competitive-comparison.md)。

想看毫无保留的版本——真实的优势、真实的当前局限，以及基于竞品对比的具体改进点？参见
[`references/honest-assessment.md`](../../../references/honest-assessment.md)。

## 快速开始

今天已有二十四个产品技能可用（[今天你能获得什么](#今天你能获得什么)）——完整的
...
| 改动规模 | 使用 | 产出 |
|---|---|---|
| 小型、已充分理解——一个 typo、一个单文件的修复、一个范围明确的小调整 | `specjedi-quick` ⚡ | 一份 `quick.md`，直接交付代码 |
| 更大、含糊、涉及多个子系统，或需要新建 `specjedi-*` 技能的改动 | 完整流水线（下面第 3-11 步） | `spec.md` → `plan.md` → `tasks.md` → 交付代码 |

`specjedi-quick` 自己会先对照五条明确标准检查是否符合资格，再动笔写任何东西——如果你的
请求实际上写不进大约一页笔记，它会拒绝并把你引导到 `specjedi-specify`，而不是勉强把它塞
进这条路径。两条路径执行的是同样的质量关卡（涉及代码时测试先行、PR 开启前的
`specjedi-govcheck`）——"quick" 只缩短规划环节，绝不缩短验证。
