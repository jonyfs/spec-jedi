<!-- i18n-sync: source=README.md@01f83f5 lang=zh -->
...
| `specjedi-diagram` 📊 | 从既有的 `spec.md`/`plan.md` 生成经渲染验证的 Mermaid 图（流程图、时序图或 ER 图——依内容推断）——始终是原文的补充，绝不取而代之 |
...
目前只构建并测试了 `-harness claude-code`（默认值）；其他值会被报告为尚不支持,而不是被悄悄
尝试执行——见下方[受支持的宿主环境](#受支持的宿主环境)。运行 `./scripts/install.sh --help`
（或 `.\scripts\install.ps1 -Help`）查看完整选项列表。
...
14. 想要一张图而不是一堵文字墙？`specjedi-diagram` 📊 会把规格说明或计划转化为经渲染验证的
    Mermaid 图——流程图、时序图或 ER 图，视实际内容而定——始终与原文并列展示，绝不取而代之。
