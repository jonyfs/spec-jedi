...
|---|---|---|---|---|
| **spec-kit** (baseline) | The SDD toolkit this project originally bootstrapped itself with (`speckit-*`), until feature 048 (2026-07-18) completed the migration to this project's own `specjedi-*` pipeline | Phase structure (constitution → specify → clarify → plan → tasks → implement → analyze/checklist/converge) and the constitution-as-source-of-truth pattern — proven, and this repo's own `specjedi-*` skills still follow the same phase structure | Rigid phase gates with no lightweight path; no guided next-step prompting (Principle XIV exists specifically because spec-kit doesn't do this); no persona/tone layer; no proactive skill-gap detection; not brandable as a distinct product | Public, ~93K GitHub stars |
...
| **Spec Kitty** | Smaller/newer entrant; git-worktree-based parallel spec development across multiple agents | Worktree-awareness itself, now mechanized as `specjedi-worktree` (feature 032). **specs/056** (2026-07-19) inspected it further via published sources and confirmed it genuinely supports multi-agent parallel dispatch (Claude Code/Codex/Cursor/Copilot/Gemini/Windsurf, one worktree per agent) — `specjedi-parallel` adopts the same worktree-as-concurrency-primitive choice | Its Kanban-dashboard/manual-lane-assignment model — `specjedi-parallel` determines safety via file-overlap analysis on a candidate's own `plan.md` scope instead of relying on a human to assign work to lanes | Newer entrant, tracked via `cameronsjo/spec-compare`; less independent adoption signal than the others |
| **Superpowers** | MIT skill collection with a mandatory-invocation meta-rule ("if a skill applies, you don't have a choice") | The "brainstorm before spec" instinct — `specjedi-specify` welcomes an unstructured idea rather than requiring a pre-formed feature description; and, inspected directly (not just cited) during feature 032, the `using-git-worktrees` skill's native-tool-first/portable-`.worktrees/`-fallback design, adopted into `specjedi-worktree` rather than reinvented | A separate mandatory meta-skill enforcement layer — Principle XIV (guided next-step suggestion) already covers "tell the user/agent what's next" without a second enforcement mechanism on top | Public, MIT — installed locally and inspected firsthand in this environment |
| **GSD** | Meta-prompting SDD system; wave-based context management | The `--auto` flag concept (ask what's needed up front, then don't stop) and the cross-harness `<runtime_note>` mapping pattern — both built directly into `specjedi-*` skills | A second, parallel `.planning/` directory convention — `specjedi-*` skills keep using `specs/NNN-feature/` to interoperate with the existing spec-kit-derived scaffolding instead of forking it. **specs/052/056** (2026-07-19) researched GSD's own durable, DB-backed resumable auto-mode state machine and its dependency-graph wave scheduling further — both rejected as heavier than this project's scope: `specjedi-chain` stays a thin sequencer with no crash-recovery state, and `specjedi-parallel` determines safety by file-overlap, not an automatic dependency graph | Installed locally and inspected firsthand in this environment |
