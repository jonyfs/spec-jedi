...
| **Spec Kitty** | Smaller/newer entrant; git-worktree-based parallel spec development | Worktree-awareness itself, now mechanized as `specjedi-worktree` (feature 032) — no longer just a *documented option*; Spec Kitty's own mechanism was never independently inspected (smaller/newer entrant), so no claim is made about its internals beyond the pattern's existence | No other design choices adopted — not enough independent adoption signal yet to treat them as proven | Newer entrant, tracked via `cameronsjo/spec-compare`; less independent adoption signal than the others |
