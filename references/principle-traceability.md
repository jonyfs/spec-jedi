...
| IX | Mandatory Skill Validation & Testing | 🟡 Partial | `scripts/validate.sh`/`.ps1` (structural lint) + `.github/workflows/validate.yml` (`lint`, `install-test` jobs, matrixed across OSes). Unit/integration/Playwright jobs: not yet in the battery — this project has shipped no unit-testable logic or web UI yet, but that exemption isn't formally documented anywhere (see CHK004). |
