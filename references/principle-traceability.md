...
| III | Universal LLM & Harness Compatibility | 🟡 Partial | README's "Supported harnesses" table names all 20 harnesses individually; `references/harness-capability-notes.md` documents a real per-harness capability matrix. **Codex CLI is now the second real, CI-proven, installable harness** (feature 016) — `scripts/install.sh`/`.ps1 --harness codex-cli` installs to `.agents/skills/`, verified structurally by a matrixed CI job pair (not a real Codex CLI run, honestly scoped). What remains genuinely open: 18 of 20 harnesses are still 📋 Planned, no built install path yet. |
