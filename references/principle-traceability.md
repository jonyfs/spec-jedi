...
| III | Universal LLM & Harness Compatibility | 🟡 Partial | README's "Supported harnesses" table names all 20 harnesses individually; `references/harness-capability-notes.md` documents a real per-harness capability matrix. **Four harnesses now real and CI-proven**: Claude Code (feature 001), Codex CLI (feature 016, `--harness codex-cli` → `.agents/skills/`), OpenCode (feature 017), and **Warp** (feature 018, Agent Mode's separate Skills system) — the latter two needed zero new installer code, since both harnesses' own documented discovery conventions already scan `.claude/skills/` and `.agents/skills/` directly; each verified via a dedicated CI job asserting that harness's exact rules, not just reused generic checks. Feature 018's own research also caught and corrected a real near-miss: an initial check of only Warp's Rules docs (a flat `AGENTS.md`/`WARP.md` file) missed its separate, directory-based Skills capability entirely. What remains genuinely open: 16 of 20 harnesses have no built/proven install path yet. |
